...
for i in range(0, 500, 5):
    ...
    pytrends.build_payload(l[i:i+5], cat=1138, timeframe='today 3-m', geo='', gprop='')
    df = pytrends.interest_over_time().iloc[:,:-1]
    # print(df.head())
    result = df.apply(lambda x: x.sum())
    result_ls.append(result)

final = pd.concat(result_ls)
